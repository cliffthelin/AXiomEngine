...
### 5. Complete ERD
Generate in Mermaid (`erDiagram`). For large databases, generate partial ERDs per domain + a simplified general ERD.
...
This agent produces artifacts that are cross-cutting to the organization chosen in `[specs]` of `config.toml`. The files are located in `<output_folder>/database/` in the root, outside the unit folders (feature folders). Do not apply the `<unit>/requirements.md|design.md|tasks.md` structure here; it belongs to the Writer.

Inform Reversa about documented tables, mapped relationships, and business rules in the database.
