...
> Plan for migrating data from the legacy system to the new system: mapping, transformations, ETL, data cutoff, and validation.

## Summary

- Estimated volume: <rows / GB per main entity>
- Migration window: <see `cutover_plan.md`>
- Strategy: pre-backfill + delta + cutover | single bulk load | continuous replication

## Legacy → New Mapping

| Source | Destination | Type | Notes |
|---|---|---|---|
| `<legacy schema>.tb_pedidos` | `pedidos` | renaming | type normalization |
| `<legacy schema>.tb_pedido_item` | `pedido_itens` | renaming | FK adjusted |
| `<legacy schema>.usr_x` | `usuarios` (partial) + `perfis` | splitting | extracts profile data |
...
- **Source of the rule**: <reference to `target_business_rules.md` or `discard_log.md`>
...
> Also see `cutover_plan.md`. This section covers only the data-specific part.
