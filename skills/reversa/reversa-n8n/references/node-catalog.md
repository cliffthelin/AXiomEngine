...
Quick reference used by `reversa-n8n` to interpret node types and suggest equivalent Python libraries. It's not exhaustive: it covers the most common nodes. When the node type is not listed, infer based on the name (`type` follows the pattern `n8n-nodes-base.<service>` or `@n8n/n8n-nodes-langchain.<service>`) and the `parameters`.
...
## 1. Triggers (Workflow Entry Point)
...
| `n8n-nodes-base.executeWorkflowTrigger` | Sub-workflow triggered by another | Callable Python function |
| `n8n-nodes-base.errorTrigger` | Triggered on another workflow's error | Exception hook, error decorator |
...
| `n8n-nodes-base.respondToWebhook` | Respond to received webhook | Return from the FastAPI/Flask route |
| `n8n-nodes-base.graphql` | GraphQL query | `gql` or `httpx` direct |
| `n8n-nodes-base.webhook` (output) | Webhook confirmation | Response from the route |
...
| `n8n-nodes-base.merge` | Merge of parallel branches | Combining lists, `dict.update`, `pandas.concat` |
| `n8n-nodes-base.splitInBatches` | Batch processing | `itertools.batched` (3.12+) or manual loop |
| `n8n-nodes-base.itemLists` | List operations (split, aggregation) | List comprehensions, `itertools` |
...
| `n8n-nodes-base.stopAndError` | Stop with an error | `raise Exception(...)` |
| `n8n-nodes-base.executeWorkflow` | Call another workflow | Function/module Python call |
...
| Node Type in JSON | Meaning | Python |
|---|---|---|
| `n8n-nodes-base.set` | Define values in fields | Assignment in a dict |
| `n8n-nodes-base.editFields` | Edit the item's structure | `dict.update`, list comprehension, dataclasses |
...
| `n8n-nodes-base.code` | JS or Python code (newer version) | Pure Python function. Verify `parameters.language` |

When translating Function/Code nodes to the `design.md`: describe the logic in pseudocode, don't copy the literal JS. The Python equivalent should be idiomatic.
...
| Node Type in JSON | Meaning | Python |
|---|---|---|
| `n8n-nodes-base.postgres` | PostgreSQL | `psycopg[binary]` (v3) or `asyncpg` |
...
| `n8n-nodes-base.whatsApp` | WhatsApp Business API | Direct `httpx` requests (no mature official Python SDK) |
...
For new projects, consider if LangChain/LangGraph is necessary or if a direct call to the LLM SDK is simpler.
...
| N8N Credentials Type | Meaning | Python Pattern |
|---|---|---|
...
| `httpQueryAuth` | API key in the query string | Fixed param in all calls |
| `oAuth2Api` | OAuth2 (with refresh) | `authlib` or `requests-oauthlib`, persist the refresh token |
...
| N8N Behavior | Python |
|---|---|
| `continueOnFail` in the node | `try/except` that logs and continues |
| `retryOnFail` with retries | `tenacity` (decorator `@retry`) |
...
- Node versions (`typeVersion`) can change internal parameters. Verify that the `parameters` structure matches the version.
- Community nodes (that start with `n8n-nodes-<community>`) may not have a direct Python equivalent. Handle these on a case-by-case basis.
```