...
Map of origin→target pairs supported in v1, with the recommended mode as the default and the canonical spec format to use in `target_screens.md`. Pairs not listed return `EC-01` and offer a raw template.
...
| `android-xml-java` | `compose` | modernized (idiom close) | `android_xml__compose` | `composable` |
| `android-xml-kotlin` | `compose` | modernized (idiom close) | `android_xml_kt__compose` | `composable` |
| `ios-xib-objc` | `flutter` | modernized | `ios_xib_objc__flutter` | `composable` |
| `ios-xib-objc` | `swiftui` | modernized (idiom close) | `ios_xib_objc__swiftui` | `composable` |
| `ios-xib-swift` | `swiftui` | modernized (idiom close) | `ios_xib_swift__swiftui` | `composable` |

## Available Modes by Pair

For each pair, generally three modes are presented to the user, but some combinations have an unviable literal mode. The table below restricts this.
...
| `delphi-vcl` → `web-spa` | partial | only with screenshot of the legacy and explicit acceptance; pixel-perfect rare |
...
### `component-tree` (desktop/web/mobile graphic, modernized)
...
Target implementation: framework-agnostic (React, Vue, Svelte, SwiftUI, Compose, Tauri webview, etc.) unless `target_architecture.md` has fixed a specific framework.

### `route-component` (modernized web from server-rendered)

Includes `spec.route` (canonical URL of the target) and `spec.layout` (parent layout). Body is a `component-tree`. `spec.api_changes` lists changes in the HTTP contract between legacy and target (URL, method, content-type), referencing deviations.
...
When the adapter does not cover the pair. Content is structured prose with mandatory sections (identity, layout, fields, messages, events, validations). Each screen in `raw-prose` must have a deviation registered, indicating that the encoder will need to interpret the prose.

## Special Inputs and States

Every spec, in any kind, can include:

- `spec.normalize`: accepted rules in comparison to the golden file (line endings, trailing spaces, trim ANSI, etc.).
...
New pairs can be added as rows in this table, along with a descriptive adapter (not code, but a textual heuristic used by the agent to generate the spec).
