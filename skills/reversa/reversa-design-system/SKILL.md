...
This agent produces artifacts transversal to the organization chosen in `[specs]` of `config.toml`. The files are located in `<output_folder>/design-system/` at the root, outside the unit folders (feature folders). Do not apply here the structure `<unit>/requirements.md|design.md|tasks.md`, it belongs to the Writer.
