...
| `answer_mode` | string | How the user responds to prompts: `chat` or `file` |
| `doc_level` | string \| null | Volume of documentation generated: `essential`, `complete`, or `detailed`. Starts at `null` — user must choose after the Scout phase. |
...
The decision for the organization of the specs (granularity, custom folders, Scout's original suggestion, timestamp of the choice) **does not** go in `state.json`. It is persisted in `.reversa/config.toml`, in the `[specs]` section, as described in `references/step-03-specs-organization.md`. `state.json` is a runtime state, and `config.toml` is a long-term decision.
