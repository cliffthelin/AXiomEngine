...
description: Conducts a guided interview of up to ten questions and generates the user's billing profile, including country, currency, normalized seniority, hourly rate, project markup, tax regime, billing model, and customer profile. Use when the user types "/reversa-pricing-profile", "reversa-pricing-profile", "configure billing profile", "set hourly rate", or asks to configure pricing in Reversa.
license: MIT
...
2.  Use simple, everyday language in Brazilian Portuguese.
3.  Do not provide formal financial, legal, or tax advice.
4.  Do not consult external networks, WebSearch, or services.
5.  Do not invent financial values; only the user provides the information.
6.  Do not use hyphens in any text. Use commas, colons, or rewrite.
7.  All writing to disk is atomic, using a temporary file and rename, UTF-8 without BOM.

## Before you begin

1.  Read `.reversa/state.json` to resolve `output_folder`. If missing, assume `_reversa_sdd/`.
...
Introduce yourself in two short sentences and state that there will be between 8 and 10 questions. Ask the questions in the order below, waiting for a response before the next.
...
Text: "What is the seniority level of your work or your team? Choose one: junior, mid, senior, staff\_lead, principal. If you prefer, you can answer 'pleno' for mid or 'especialista' for staff\_lead."
...
Text: "How do you want to specify your hourly rate? Choose one: 1) direct mode, I already know the value. 2) derived mode, calculate it based on the desired monthly income and billable hours."

If the user chooses "direct":

1.  Ask: "What is your net hourly rate in local currency? Just the number."
2.  Save `hourly_rate_mode = "direto"`, `hourly_rate = <value>`, `monthly_target_income = null`, `billable_hours_per_month = null`.
...
Save in `margin_percent` for historical compatibility, but explain that the field means project markup, not net accounting margin.
...
```
1. <key>: <name_pt_br> (estimated rate: <tax_factor * 100>%, source: <tax_factor_source>)
2. ...
...
1.  Suggest the default regime for the country, when available.
2.  Set `tax_regime_confidence = "low"`.

If the user chooses "other", save:

```
tax_regime = "other"
tax_factor = 0
tax_factor_kind = "not_computed"
tax_factor_source = "User reported non-cataloged regime"
includes_vat = false
...
Set `tax_regime_confidence = "high"` if the user explicitly chose a regime.

### Question 7: Billing models

Text: "Which billing models do you use? You can choose more than one, separated by commas. Options: fixed\_scope, time\_and\_materials, sprint, retainer, fixed\_price\_per\_delivery."

At least one model is mandatory. Save in `pricing_models`.

### Question 8: Customer profile

Text: "What customer profile do you serve? You can choose more than one, separated by commas. Options: microenterprise, small\_business, medium\_business, enterprise, government, international\_client."

Accept an empty answer or "skip." In this case, save an empty array.
...
-   Project markup
-   Tax regime, approximate rate, rate type, and source
-   Warning if the rate includes VAT, IVA, ISS, or tax
-   Billing models
-   Customer profile
...
Mentally validate against the schema. If something is missing, only re-ask the corresponding question.

Save `_reversa_sdd/_pricing/profile.json` and `_reversa_sdd/_pricing/profile.md` atomically.

## Disclaimer of profile.md

Include:

```
Disclaimer: The registered tax rate is an approximate estimate for budgeting, not an exact legal rate. Real tax validation is the responsibility of the user's accountant. This file contains sensitive financial data. It is recommended to add `_reversa_sdd/_pricing/profile.json` and `_reversa_sdd/_pricing/profile.md` to the `.gitignore` before committing.
```
...
3.  Path of the backup, if an overwrite occurred.
4.  Next step:
