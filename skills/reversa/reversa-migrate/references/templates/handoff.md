...
> This document is the entry point for the coding agent (Claude Code, Codex, Cursor, Antigravity, etc.) that will write the new system based on the specifications.

## ⚠️ Mandatory Reading First

1. **`paradigm_decision.md`**, non-negotiable reading. The target paradigm shapes how all coding should take place.
...
> Items that require human decision before the coding agent begins.
...
1. **Read and internalize `paradigm_decision.md`**: the target paradigm is <from paradigm_decision>. All code choices must honor this paradigm.
2. **Read and internalize `topology_decision.md`**: the chosen topology is <preserve | modernize | hybrid>. Use the outline of the tree recorded in this artifact as a basis to create the folder structure of the new repository.
3. **Read and internalize `screen_modernization_decision.md`** (when there is a UI): the screen translation mode is <literal | modernized | hybrid>. In literal mode, materialize byte-for-byte (or pixel-equivalent) what is in `target_screens.md`; in modernized mode, honor the hierarchy of components, tokens, and the 4 states (idle, loading, error, success).
...
6. **Implement the screens** consuming `target_screens.md` as a literal contract. In literal mode with golden files present in `_reversa_sdd/screens/golden/`, the result of the implementation must match the golden file within the `normalizationRules` declared in the `manifest.yaml`.
7. **Write the tests** from `parity_specs.md` and `parity_tests/*.feature` from the beginning. Honor the § Exceptions section, which reflects deviations approved in `screen_deviation_log.md`.
8. **For each component**, validate that it respects the chosen paradigm (explicit signals in `target_architecture.md § Honor the chosen paradigm`) and the chosen topology (explicit signals in `target_architecture.md § Honor the chosen topology`).
...
> List here items where the default was applied without human confirmation. It is recommended to review before the cutover.

- <or: pipeline executed in interactive mode, no self-decided items>

## Final Notes
<Orchestrator's notes for the coding agent.>
```