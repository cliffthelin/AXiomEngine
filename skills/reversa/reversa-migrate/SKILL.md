...
description: "Orchestrator for the Reversa Migration Team. Guides the migration pipeline after `/reversa` has populated `_reversa_sdd/`. Collects a brief, invokes the 6 agents (Paradigm Advisor → Curator → Strategist → Designer → Screen Translator → Inspector) with human pauses, and generates the final `handoff.md`. Use when the user enters `/reversa-migrate`, `reversa-migrate`, `migrate system`, or `start migration`."
license: MIT
...
Migration is a **subsequent step** to the main Reversa workflow. The user first runs `/reversa` on the legacy system, which triggers the Discovery Team (Scout → Archaeologist → Detective → Architect → Writer → Reviewer) and populates `_reversa_sdd/`. Only after this step can `/reversa-migrate` run.
...
3.  For each artifact `required: true`, verify its presence in `_reversa_sdd/` (also consider declared aliases).
    *   If any are missing: list all missing artifacts, inform that the pipeline is blocked, ask the user to run `/reversa` again, and terminate.
...
2.  If it exists: read it. Identify `currentAgent.agent`, `currentAgent.phase`, `currentAgent.status`, `completedAgents`.
    *   **Special case: pending intra-agent pause.** If `currentAgent.status == "awaiting_user_approval"` (typical after Designer Phase 1, session closed before approval): re-read the artifact in pause ( `topology_decision.md` when `phase == "topology"`), reconstruct the 3 to 8-line summary using the template for the corresponding agent's step, and re-execute the human pause before continuing. Do not offer a menu of options until the pause is resolved.
    *   **Normal case**, ask the user:
        ...
        > 3. Recreate starting from a specific agent
        > 4. Cancel"
...
If `_reversa_sdd/migration/migration_brief.md` **does not exist**, conduct the interview; otherwise, offer `review / maintain / recreate`.
...
Create `_reversa_sdd/migration/.state.json` from the template in `references/state.json`. Fill in `startedAt`, `engine`, `reversaVersion`. Set `currentAgent.agent = "paradigm_advisor"`, `currentAgent.phase = null`, `currentAgent.status = "running"`, `currentAgent.topologyApproved = false`.
...
2.  Activate the agent's skill (`reversa-paradigm-advisor`, `reversa-curator`, `reversa-strategist`, `reversa-designer`, `reversa-screen-translator`, `reversa-inspector`). If the engine does not support direct activation by name, instruct to read `.agents/skills/<id>/SKILL.md` in the current context.
...
5.  **Human pause** (see Step 6) before proceeding, as shown in the table below.

#### Step 5b: Intra-agent checkpoint

Some agents operate in phases with a human pause between them. Today, **Designer** and **Screen Translator** behave this way. Each declares its own phases in the "Phase Detection on Start" section of the SKILL.md, and uses a `<artifact>Approved` field in `currentAgent` as the single source of truth for approval.
...
4.  After approval, orchestrator sets `currentAgent.<artifact>Approved = true`. This is the single source of truth; do **not** duplicate it in the artifact's front-matter.
...
6.  Upon completing Phase 2, the agent returns control with `status: complete` (or `skipped` in the case of the Screen Translator in a legacy system without a UI). The orchestrator runs the corresponding pause in the table.
7.  If the user requests adjustments in either of the two phases, the orchestrator re-activates the agent, explicitly indicating which phase should be redone:
    ...
    *   Screen Translator: `--regenerate-phase=mode` or `--regenerate-phase=generation`.
    The agent respects the instruction and discards artifacts from the phase onwards.
...
| Screen Translator (Phase 1) | Approve `screen_modernization_decision.md` (literal / modernized / hybrid). In Hybrid mode, explicit lists of screens per mode are required. In a legacy system without a UI, the agent skips without a pause. |
...
*   **Engines without interactive TTY**: write `_reversa_sdd/migration/pending_decisions.md` with the open decisions, instruct the user to edit and signal completion, and re-read the file after signaling.
...
After each agent, integrate ⚠️ items and pending items in `_reversa_sdd/migration/ambiguity_log.md`. In the end, organize them into three groups:
...
4.  List items in the "REFERRED TO CODING" section.
5.  Add next steps specific to the coding agent (set up new repository, implement bottom-up, validate parity, run cutover).
...
Save the complete log in `_reversa_sdd/migration/.logs/<timestamp>-migrate.log` with timestamp per entry and agent identification. If the engine exposes token count or cost, record it; if not, leave the fields blank without invalidating the log.
...
3.  If `currentAgent.status == "awaiting_user_approval"`, follow the special case of Step 2 (re-execute the pending pause). Otherwise, confirm with the user before resuming.
4.  Continue from the next agent (or from the same if it was `failed`, or from the next phase if it was `awaiting_user_approval` and it was resolved).
...
3.  Delete artifacts:
    *   `--regenerate=<agent>`: artifacts of the specified agent **and all subsequent agents** in the pipeline order. For the Designer, this includes `topology_decision.md` and resets `currentAgent.topologyApproved = false`. For the Screen Translator, this includes `screen_modernization_decision.md`, `target_screens.md`, `screen_deviation_log.md`, `_reversa_sdd/screens/inventory.json`, and `_reversa_sdd/screens/golden/`, and resets `currentAgent.screenModeApproved = false`.
    ...
    *   `--regenerate=designer:architecture`: deletes only the artifacts from the Designer's Phase 2 (`target_architecture.md`, `target_domain_model.md`, `target_data_model.md`, `data_migration_plan.md`). Preserves `topology_decision.md` and `topologyApproved`.
...
*   **Manual modification of a generated artifact** (hash in `.state.json` is different): pause, present a summarized diff, and offer (a) preserve the modified version and abort regeneration, (b) overwrite with the backup, (c) abort the pipeline. `--auto` defaults to (a).
*   **LLM failure in the middle of an agent**: state preserved, agent is marked as `failed`. `--resume` re-executes this agent.
*   **Designer asked for adjustments** after reviewing the architecture: re-run the Designer at the same step, without moving on to the Inspector.
...
*   Each pause presents a summary + pending decisions; it never proceeds silently.
