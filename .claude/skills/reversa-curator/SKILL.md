...
To decide, rule by rule, what to migrate to the new system, what to discard, and what requires human decision, based on three critical inputs:
...
-   `_reversa_sdd/<unit>/requirements.md` and `_reversa_sdd/<unit>/design.md` for each unit (specifications per unit, containing business rules)
...
Apply in this order (the first match determines the decision):

1.  **Rule ⚠️ AMBIGUOUS** or **🔴 GAP** → HUMAN DECISION. List in the dedicated section of `target_business_rules.md` and replicate a summary in `ambiguity_log.md`.
2.  **Rule incompatible with `migration_brief.md`** (excluded scope, technical restriction that invalidates, regulation that changes) → DISCARD with explicit justification.
3.  **Rule that is an artifact of the legacy paradigm and not of the business** (see the list of examples below) and the paradigm has changed → DISCARD, noting the link to the paradigm in `discard_log.md`.
4.  **Rule cited in `pain_points.md` / `gaps.md` as a problem** → HUMAN DECISION with a Curator recommendation.
5.  **Rule 🟡 INFERRED** → MIGRATE with a warning for validation in the coding agent.
...
-   Manual pessimistic lock via `SELECT ... FOR UPDATE` in a synchronous, procedural legacy system → in the target event-driven system, idempotency via event ID replaces the lock.
-   Distributed transaction via 2PC in a classic OO legacy system → in the target event-driven system, it becomes a saga with compensation.
...
Fundamental decision: **a rule is discarded when the new paradigm absorbs the use case by design, without the need for the old manual mechanism.** Do not discard simply because it is "another way of doing things" if the business rule itself continues to exist.

## Procedure

### 1. Read Artifacts

Read `paradigm_decision.md` in its entirety (especially "Pending Implications for Subsequent Agents") and `migration_brief.md`. Then, in each unit folder within `_reversa_sdd/`, read the `requirements.md` and `design.md` files, plus the auxiliary artifacts.

### 2. Inventory Rules

Internally construct a list of business rules found. Each rule should have:
...
> "- HUMAN DECISION: <n>"
>
> "Next step: review the HUMAN DECISION items. Next agent: **Strategist**."

## Edge Cases

-   **Unit folders missing or incomplete in `_reversa_sdd/`** (Writer did not run, or ran partially): treat `domain.md` and `code-analysis.md` as sources; explicitly state in the summary that the granularity is limited by the quality of `_reversa_sdd/`.
-   **Rule duplicated across components**: consolidate into a single `BR-MIGRATE-XXX` with multiple origins.
-   **Rule that is partially affected by the paradigm**: prefer to MIGRATE + note of "compatibility with the target paradigm" instead of DISCARD.
