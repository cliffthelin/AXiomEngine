...
Use this scale in **every** statement within the specifications. No exceptions.

## Definitions

| Symbol | Name | Meaning |
|---|---|---|
| 🟢 | CONFIRMED | Extracted directly from the code — can be cited with file and line number |
| 🟡 | INFERRED | Deduced from patterns, names, conventions, or context — may be incorrect |
| 🔴 | GAP | Could not be determined from the code — requires human validation |

## When to use each level

### 🟢 CONFIRMED
- The behavior is explicitly present in the code (if/else, return, throw)
...
- There are indications in the code, but the complete logic is not visible within the analyzed scope
...
- Business rule that only exists in the minds of the stakeholders
...
### Downgrade: 🟡 → 🔴
Conditions: find evidence that the inference was incorrect.
