...
Map of supported source → target pairs in v1, with the recommended mode as the default and the canonical specification format to be used in `target_screens.md`. Pairs not listed return `EC-01` and offer a raw template.

## Master Table

| Source | Target | Recommended Mode | Adapter | Specification Format |
|---|---|---|---|---|
...
| `android-xml-java` | `compose` | modernized (closely related language) | `android_xml__compose` | `composable` |
| `android-xml-kotlin` | `compose` | modernized (closely related language) | `android_xml_kt__compose` | `composable` |
| `ios-xib-objc` | `flutter` | modernized | `ios_xib_objc__flutter` | `composable` |
| `ios-xib-objc` | `swiftui` | modernized (closely related language) | `ios_xib_objc__swiftui` | `composable` |
| `ios-xib-swift` | `swiftui` | modernized (closely related language) | `ios_xib_swift__swiftui` | `composable` |
...
| `delphi-vcl` → `web-spa` | partial | only with a screenshot of the legacy application and explicit acceptance; pixel-perfect rendering is rare |
| `win32-mfc` → `web-spa` | no | rejects literal mode; recommends modernized |
| `android-xml-*` → `flutter` | partial | only with screenshots per density; pixel-perfect rendering depends on the font |
| `android-xml-*` → `compose` | partial | same language, closest option, but widgets diverge |
...
Typical target implementation: a function per screen in `pkg/menu/screens.<ext>` that writes to an `io.Writer`.

### `component-tree` (desktop/web/mobile graphical, modernized mode)
...
Target implementation: framework of choice (React, Vue, Svelte, SwiftUI, Compose, Tauri webview, etc.), unless `target_architecture.md` has already fixed a specific framework.

### `route-component` (modernized web from server-rendered)

Includes `spec.route` (canonical URL of the target) and `spec.layout` (parent layout). Body is a `component-tree`. `spec.api_changes` lists changes in HTTP contract between legacy and target (URL, method, content-type), referencing deviations.
...
When the adapter does not cover the pair. Content is structured prose with required sections (identity, layout, fields, messages, events, validations). Each screen in `raw-prose` must have a deviation registered, indicating that the encoder will need to interpret the prose.

## Special Inputs and States

Every specification, in any kind, can include:

- `spec.normalize`: rules accepted in comparison with the golden file (line endings, trailing spaces, trim ANSI, etc.).
- `spec.interpolations`: points where dynamic data from the domain enters (e.g., `{{titular}}`, `{{saldo}}`). With types and restrictions (max_width, regex, lookup).
- `spec.transitions`: list of events that lead to another screen.
- `spec.legacy_origin`: path `file:line` or `file:paragraph` in the legacy application.
...
New pairs can be added as rows in this table, along with a descriptive adapter (not code, but a textural heuristic used by the agent to generate the specification).
