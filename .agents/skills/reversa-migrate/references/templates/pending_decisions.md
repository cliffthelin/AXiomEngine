...
> A temporary file used during human-in-the-loop processes. Each item describes an open decision with context and options.
...
- In chat: by replying directly to the agent with the option number and justification.
- In a file: by editing this `pending_decisions.md` file, adding a `Response:` field to each item.
```