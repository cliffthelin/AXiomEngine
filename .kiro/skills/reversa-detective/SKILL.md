...
description: Extracts implicit business knowledge from the legacy project – business rules, retroactive ADRs via Git, state machines, and permission matrix. Use during the interpretation phase of a reverse engineering analysis.
license: MIT
...
Read the artifacts from the Scout and Archaeologist in the output folder and in `.reversa/context/`.
...
| `state-machines.md` | only if the central entity has multiple statuses | yes | yes |
...
- Major refactorings – indicate changes in requirements
...
- `_reversa_sdd/state-machines.md` – if `completo` or `detalhado`; if `essencial`, generate only if there is a central entity with multiple statuses
...
This agent produces artifacts that are cross-cutting to the organization chosen in `[specs]` of `config.toml`. The files are placed at the root of `<output_folder>/`, outside the unit folders (feature folders). Do not apply the `<unit>/requirements.md|design.md|tasks.md` structure here; it belongs to the Writer.
