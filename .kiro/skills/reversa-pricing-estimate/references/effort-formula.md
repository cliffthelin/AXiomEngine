...
This document describes the deterministic calculation that the `reversa-pricing-estimate` agent applies to the Effort scenario. Version 2 removes the old linear score-to-hours conversion and uses hourly ranges per T-shirt size, with a seniority factor inspired by the COCOMO II personnel capacity multipliers.

## Source and Criteria

COCOMO II is a parametric software effort estimation model that uses size, product attributes, platform, personnel, and project. For Reversa UX, using the complete model would be too complex. V2 only uses the defensible idea of personnel capacity multipliers, maintaining simple hourly ranges per class.
...
hours_by_complexity_class_senior:
  ...
  L:   32 to 80 hours
  XL:  80 to 160 hours
  XXL: 160 to 320 hours, with a mandatory recommendation to break down the scope
```
...
The `horas_estimadas` field is the midpoint for compatibility and summary. The `horas_min` to `horas_max` range should be displayed in the estimate.md.
...
When `profile.tax_regime == "other"` or `tax_factor = 0`, the tax is not calculated, and estimate.md should display an explicit warning.

If the profile indicates that the factor includes VAT, IVA, or tax itemized on the invoice, estimate.md should warn that this value may be passed on to the client and does not necessarily reduce the margin.

## Step 6: Project markup

The historical `margin_percent` field should be treated as **project markup on direct cost**, not as accounting net profit margin.
...
2.  XXL is still calculable, but should generate a strong recommendation to break down the scope.
3.  The hourly range is a heuristic, not a delivery promise.
4.  `size_score` does not enter into the hour calculation.